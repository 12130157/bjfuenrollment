...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (username.isEmpty() || nickname.isEmpty() || password.isEmpty()) {
			...
			session.setAttribute("error",
					"<font color='red'>对不起,密码长度不在6~20之间！</font>");
			request.getRequestDispatcher("register.jsp").forward(request,
					response);
			...
			UserVO uvo = new UserVO(0, username,password, nickname, 0, "",
					time, uip, 0);
			...
			if (suc == null) { // 成功
				...
				request.getRequestDispatcher("login.jsp").forward(request,
					...
					session.setAttribute("error", "数据库连接失败！");
					request.getRequestDispatcher("register.jsp").forward(request,
							response);
				}
			} else {
				session.setAttribute("error",
						"<font color='red'>对不起，该用户名已存在！</font>");
				request.getRequestDispatcher("register.jsp").forward(request,
						response);
			}
		}
